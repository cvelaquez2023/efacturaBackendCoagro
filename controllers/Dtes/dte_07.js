  ...
  try {
    ...
    if (_dteProcesado.length > 0) {
      if (_dteProcesado[0].estado == "PROCESADO") {
        logger.info("documento ya fue Procesado", { controller: "dte_07", action: "postDte07" });
    ...
    if (!HayContingencia) {
      ...
      const postrecepciondte = async () => {
        try {
          ...
          logger.error("Error en postrecepciondte", { controller: "dte_07", action: "postrecepciondte", stack: error.stack });
        }
      };
      ...
      if (
        ...
        if (_respuestaMH.estado === "RECHAZADO") {
          try {
            ...
            logger.error("Error procesando rechazo", { controller: "dte_07", action: "postDte07_rechazo", stack: error.stack });
          }
        } else if (_respuestaMH.estado == "PROCESADO") {
          await generaPdf.generaPdf07(_factura);
          setTimeout(function () {
            logger.info("procedemos a enviarlos", { controller: "dte_07", action: "postDte07" });
          }, 1000);
    ...
    await notifyError("dte_07", "postDte07", error, "Factura: " + req.params.factura);
